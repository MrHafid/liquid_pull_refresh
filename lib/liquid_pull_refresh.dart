...
class LiquidPullRefresh extends StatefulWidget {
  ...
  @override
  LiquidPullRefreshState createState() => LiquidPullRefreshState();
}

class LiquidPullRefreshState extends State<LiquidPullRefresh>
    with TickerProviderStateMixin<LiquidPullRefresh> {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    if (notification is ScrollStartNotification &&
      ...
      setState(() {
        _mode = _LiquidPullRefreshMode.drag;
      });
      return false;
    }
    ...
    if (indicatorAtTopNow != _isIndicatorAtTop) {
      if (_mode == _LiquidPullRefreshMode.drag ||
          _mode == _LiquidPullRefreshMode.armed) {
        _dismiss(_LiquidPullRefreshMode.canceled);
      }
      ...
      if (_mode == _LiquidPullRefreshMode.armed &&
      ...
      switch (_mode) {
        case _LiquidPullRefreshMode.armed:
          _show();
          break;
        case _LiquidPullRefreshMode.drag:
          _dismiss(_LiquidPullRefreshMode.canceled);
          break;
        default:
          // do nothing
  ...
  Future<void> _dismiss(_LiquidPullRefreshMode newMode) async {
    await Future<void>.value();
    // This can only be called from _show() when refreshing and
    // _handleScrollNotification in response to a ScrollEndNotification or
    // direction change.
    assert(newMode == _LiquidPullRefreshMode.canceled ||
        newMode == _LiquidPullRefreshMode.done);
    setState(() {
      _mode = newMode;
    });
    switch (_mode) {
      case _LiquidPullRefreshMode.done:
      ...
      case _LiquidPullRefreshMode.canceled:
        await _positionController.animateTo(0.0,
  ...
  void _checkDragOffset(double containerExtent) {
    assert(_mode == _LiquidPullRefreshMode.drag ||
        _mode == _LiquidPullRefreshMode.armed);
    double newValue =
  ...
  void _show() {
    assert(_mode != _LiquidPullRefreshMode.refresh);
    assert(_mode != _LiquidPullRefreshMode.snap);
    final Completer<void> completer = Completer<void>();
    _pendingRefreshFuture = completer.future;
    _mode = _LiquidPullRefreshMode.snap;
    ...
    _springController
      ...
      if (mounted && _mode == _LiquidPullRefreshMode.snap) {
        setState(() {
          // Show the indeterminate progress indicator.
          _mode = _LiquidPullRefreshMode.refresh;
        });
        ...
        refreshResult.whenComplete(() {
          if (mounted && _mode == _LiquidPullRefreshMode.refresh) {
            completer.complete();

            _dismiss(_LiquidPullRefreshMode.done);
          }
        });
      }
    });
  ...
  Future<void>? show({bool atTop = true}) {
    if (_mode != _LiquidPullRefreshMode.refresh &&
        _mode != _LiquidPullRefreshMode.snap) {
  ...
  Widget build(BuildContext context) {
    ...
    double heightLoader =
        (widget.heightLoader != null) ? widget.heightLoader! : widget.height!;
            ...
            builder: (BuildContext buildContext, Widget? child) {
              return Align(
                ...
                child: widget.loaderWidget != null
                    ...
                    //         15 /
                    //         100 * // 15.0
                    //         ((_mode != _LiquidPullRefreshMode.done)
                    //             ? _indicatorRadiusWithPeakAnimation.value
                    //             : _radiusAnimation.value),
                    //     heightLoader: heightLoader *
                    //         15 /
                    //         100 * // 15.0
                    //         ((_mode != _LiquidPullRefreshMode.done)
                    //             ? _indicatorRadiusWithPeakAnimation.value
                    //             : _radiusAnimation.value),
                    //     imageAssets: widget.loaderWidget!)
                        ...
                        children: [
                          Transform(
                            transform: Matrix4.identity()
                              ..rotateZ(_progressingRotateAnimation.value *
                                  5 *
                                  pi /
                                  6),
                            ...
                            child: CircularProgress(
                              ...
                              innerCircleRadius: heightLoader *
                                  15 /
                                  100 * // 15.0
                                  ((_mode != _LiquidPullRefreshMode.done)
                                      ? _indicatorRadiusWithPeakAnimation.value
                              ...
                              progressPercent:
                                  _progressingPercentAnimation.value,
                            ),
                          ),
                          Center(
                            child: Container(
                                ...
                                decoration: BoxDecoration(
                                    ...
                                    borderRadius: BorderRadius.circular(
                                        heightLoader *
                                            ...
                                            100 * // 15.0
                                            ((_mode !=
                                                    _LiquidPullRefreshMode.done)
                                                ? _indicatorRadiusWithPeakAnimation
                                                    .value
                                                : _radiusAnimation.value))),
                                height: heightLoader *
                                    15 /
